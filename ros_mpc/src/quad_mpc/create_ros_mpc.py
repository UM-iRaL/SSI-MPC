...
class ROSMPC:
    def __init__(self, t_horizon, n_mpc_nodes, opt_dt, quad_name, point_reference=False, rf_dict=None):
        ...
        quad_mpc = Quad3DMPC(quad, t_horizon=t_horizon, optimization_dt=opt_dt, n_nodes=n_mpc_nodes,
                             ...
                             q_mask=q_mask, q_cost=q_diagonal, r_cost=r_diagonal,
                             rf_dict=rf_dict)
    ...
    def optimize(self, model_data, update_rff_dict=None):

        w_opt, x_opt = self.quad_mpc.optimize(use_model=model_data, return_x=True, update_rff_dict=update_rff_dict)
