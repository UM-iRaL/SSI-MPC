...
from src.utils.trajectories import loop_trajectory, tilted_loop_trajectory, wraped_loop_trajectory, random_trajectory, lemniscate_trajectory, wraped_lemniscate_trajectory
import numpy as np
import rospy
...
class ReferenceGenerator:
    ...
    def __init__(self):
        ...
        v_list = rospy.get_param('~v_list', default=[4.0])
        ...
        # loop_v_max_list = [2.5]*5 + [5.0]*5 + [7.5]*5 + [10.0]*5 + [12.5]*5 
        # loop_v_max_list = [2.0]*5 + [4.0]*5 + [6.0]*5 + [8.0]*5 + [10.0]*5 
        # n_trajectories = len(loop_v_max_list)
        # rospy.loginfo("Total number of trajectories to be sent: %i", n_trajectories)
        ...
        while not rospy.is_shutdown():
            ...
            if not self.mpc_busy and mode == "loop":
                rospy.loginfo("Sending increasing speed loop trajectory")
                # loop_v_max = loop_v_max_list[curr_trajectory_ind]
            ...
            elif not self.mpc_busy and mode == "tilted_loop":
                rospy.loginfo("Sending increasing speed tilted loop trajectory")
                # loop_v_max = loop_v_max_list[curr_trajectory_ind]
                x_ref, t_ref, u_ref = tilted_loop_trajectory(quad, opt_dt, v_max=loop_v_max, radius=loop_r, z=loop_z,
                                                      lin_acc=loop_a, clockwise=loop_cc, map_name=map_limits,
                                                      yawing=loop_yawing, plot=plot)

                msg = ReferenceTrajectory()
                msg.traj_name = "tilted_circle"
                msg.v_input = loop_v_max
                msg.seq_len = x_ref.shape[0]
                msg.trajectory = np.reshape(x_ref, (-1,)).tolist()
                msg.dt = t_ref.tolist()
            ...
            elif not self.mpc_busy and mode == "wraped_loop":
                rospy.loginfo("Sending increasing speed wraped loop trajectory")
                # loop_v_max = loop_v_max_list[curr_trajectory_ind]
                x_ref, t_ref, u_ref = wraped_loop_trajectory(quad, opt_dt, v_max=loop_v_max, radius=loop_r, z=loop_z,
                                                      lin_acc=loop_a, clockwise=loop_cc, map_name=map_limits,
                                                      yawing=loop_yawing, plot=plot)

                msg = ReferenceTrajectory()
                msg.traj_name = "wraped_circle"
                msg.v_input = loop_v_max
                msg.seq_len = x_ref.shape[0]
                msg.trajectory = np.reshape(x_ref, (-1,)).tolist()
                msg.dt = t_ref.tolist()
            ...
            elif not self.mpc_busy and mode == "wraped_lemniscate":
                rospy.loginfo("Sending increasing speed wraped lemniscate trajectory")
                # loop_v_max = loop_v_max_list[curr_trajectory_ind]
                x_ref, t_ref, u_ref = wraped_lemniscate_trajectory(quad, opt_dt, v_max=loop_v_max, radius=loop_r, z=loop_z,
                                                            lin_acc=loop_a, clockwise=loop_cc, map_name=map_limits,
                                                            yawing=loop_yawing, plot=plot)

                msg = ReferenceTrajectory()
                msg.traj_name = "wraped_lemniscate"
                msg.v_input = loop_v_max
                msg.seq_len = x_ref.shape[0]
                msg.trajectory = np.reshape(x_ref, (-1,)).tolist()
                msg.dt = t_ref.tolist()
