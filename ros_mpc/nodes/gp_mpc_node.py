...
from src.quad_mpc.gp_create_ros_mpc import ROSGPMPC
...
class GPMPCWrapper:
    def __init__(self, quad_name, environment="gazebo", recording_options=None, load_options=None, use_ekf=False,
                 rdrv=None, plot=False, reset_experiment=False, save_data=False, vis=True):
    ...
    def reference_callback(self, msg):
        ...
        rospy.loginfo("New trajectory received. Time duration: %.2f s. Max velocity: %.2f m/s. Max speed: %.4f." 
                      % (self.t_ref[-1], self.ref_v, np.max(np.linalg.norm(self.x_ref[:, 7:10],axis=1))))
            ...
            if abs(self.x[2] - 0.1) < 0.05:
                ...
                if not self.result_done:
                    ...
                    if self.save_data:
                        ...
                        data_file = os.path.join(self.data_dir, 'GPMPC_'+self.ref_traj_name+'_'+str(self.ref_v)+'.mat')
